...
export default function EventItem({ date, title, location, description, fullDetails, registrationLink }) {
  ...
  const handleRegister = (e) => {
    e.stopPropagation();
    registrationLink ? window.open(registrationLink, "_blank") : alert("Registration for this event is not yet open.");
  };

  return (
    <div className={`event-card ${isExpanded ? "expanded" : ""}`} onClick={() => setIsExpanded(!isExpanded)}>
      <div className="event-date"><span className="event-month">{month}</span><span className="event-day">{day}</span></div>
      <div className="event-content">
        ...
        <div className="event-extra">
          <p>{fullDetails}</p>
          <div className="event-actions"><button onClick={handleRegister}>Register Now</button></div>
        </div>
      </div>
    </div>
  );
